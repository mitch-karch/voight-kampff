...
def wolf_short_query(submit_q, w_token):
    fixedReq = string_formatter(submit_q)
    data = query_request(
        "api.wolframalpha.com",
        "/v1/result?appid={key}&i={r_s}".format(key=w_token, r_s=fixedReq),
        raw_return=True,
    )
    ...
    constructed_string = "**{question}?**\n" "*{answer}*".format(
        question=submit_q, answer=data
    )
    em = Embed(title="Wolfram Alpha", description=constructed_string, colour=0xF12223)
